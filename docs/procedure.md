...
<!--
  This page mirrors the root PROCEDURE.md. Keep the two identical: when you
  edit one, copy the change to the other. The body below (after the front
  matter) should match PROCEDURE.md verbatim.
-->
...
- **Path A — pick an offered topic.** Browse the TA profiles and their offered
  topics on this website. Email the relevant TA stating the topic you want.
...
2. **Pool entry** — the TA confirms you are in the pool and tells you your
   **assigned id** (a 16-character hex string). From this point your submission
   appears, by id and topic only, in the public list. Your name is never
   published.
...
3. The draw script computes, for every submission, a score and ranks each pool.
...
unknown when ids are assigned, every submission has an equal `1/n` chance of
being rank 1, where `n` is the size of that pool.
